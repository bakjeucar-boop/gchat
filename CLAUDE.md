...
- 다음: 세션 6 — `export.py` Markdown 내보내기 + 통합 시험(5절 28항목) + 배포
...
- 실측으로 해소된 것 (`docs/api_findings.md`)
  ...
  - B-8 (세션 5): 기본 인스트럭션이 출력을 64% 줄이고(1,221 → 435 토큰)
    1,536 상한 도달을 없앴다. 생성 속도는 초당 33.1 토큰
- 계획서 값과 실측이 어긋나 **보고만 하고 코드는 그대로 둔 것**: 컨트롤 바의
  턴 수 표시는 계획서 1.5절의 턴당 1,750 토큰을 쓰는데, 인스트럭션이 켜진
  기본 상태의 실측은 약 445 다 (예산 9,000 이 5턴이 아니라 약 20턴)
