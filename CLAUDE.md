...
## 계획서보다 앞서간 코드 (세션 7에서 계획서 갱신 필요)
| 절 | 계획서 | 현재 코드 |
|---|---|---|
| 1.4 | Gemma `default_max_output` 2,048 | 4,096 |
| 1.5 | 턴당 1,750 단일 값 | 용도별 (범용 1,750 / 코딩 4,296) |
| 2.4 | 휘발성 경고 상시 노출 | 화면에서 제거 |
| 2.4 | `Message` 구조 | `stopped_by_user` 추가 |
| 2.4.1 | `st.code` 대안 가능 | 원클릭 JS 복사 |
| 2.6.2 | 모델별 기본 인스트럭션 1종 | 용도 프리셋 3종 + 용도별 길이 지시 |
| 2.6.3 | 사이드바 순서 | 새 대화 → 목록 → 내보내기 → 전체 삭제 → 한도 → 설정 |
